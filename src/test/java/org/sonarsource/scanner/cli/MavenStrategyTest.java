...
class MavenStrategyTest {
  ...
  @Test
  void builds_mvn_sonar_command_with_sonar_properties() {
    RecordingCommandRunner runner = new RecordingCommandRunner(0);
    MavenStrategy strategy = new MavenStrategy(runner) {
      @Override
    ...
    props.setProperty("sonar.host.url", "https://sonarcloud.io");
    props.setProperty("other.prop", "ignored");
    strategy.execute(tempDir, props);

    List<String> command = runner.lastCommand;
    assertThat(command).containsSequence("mvn", "sonar:sonar");
    assertThat(command).contains("-Dsonar.host.url=https://sonarcloud.io");
    assertThat(command).doesNotContain("-Dother.prop=ignored");
  }
